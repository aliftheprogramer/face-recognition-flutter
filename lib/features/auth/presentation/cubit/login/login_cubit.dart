...
class LoginCubit extends Cubit<LoginState> {
  ...
  LoginCubit()
    : _signinUsecases = sl<SigninUsecases>(),
  ...
  Future<void> submit() async {
    ...
    final result = await _signinUsecases(
      param: LoginRequestModel(email: state.email, password: state.password),
    );
    result.fold(
      (err) => emit(
        state.copyWith(
      ...
      (_) async {
        ...
        // Mark first-run complete (if any) and refresh global auth state so
        // the app navigates to StartScan if token is saved.
        try {
          await sl<AuthStateCubit>().finishWelcomeScreen();
        } catch (_) {}
      },
    );
  }
